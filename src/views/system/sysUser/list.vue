  ...
  methods: {
    ...
    showAssignRole(row) {
      ...
      roleApi.getRolesByUserId(row.id).then((response) => {
        ...
        console.log("是否是不确定的", this.isIndeterminate);
      });
    },
    /*
      全选勾选状态发生改变的监听
    ...
    handleCheckAllChange(value) {
      // value 当前勾选状态true(全选）/false（全部选）
      // 如果当前全选, userRoleIds就是所有角色id的数组, 否则是空数组,将所有角色的数组中的所有id遍历到数组中
    ...
    handleCheckedChange(value) {
      ...
      console.log("角色列表选中项发生改变的监听", this);
      this.checkAll =
    ...
    switchStatus(row) {
      ...
      api.changeStates(row.id, row.status).then((response) => {
        this.$message.success(response.message || "修改用户状态成功！");
        this.fetchData(this.page);
      });
    },
    //分页每页数据控制
